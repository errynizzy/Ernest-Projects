...
$layout = new TLayout("add_basic_2col_center", "RoundedBlue_white", "MobileBlue_white");
$layout->version = 2;
...
$page_layouts["simcards_add"] = $layout;
...
{
	$templatefile = GetTemplateName("simcards", "inline_add");
}
else
{
	$templatefile = GetTemplateName("simcards", "add");
}
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing supplier_name - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_supplier_name = $pageObject->getControl("supplier_name", $id);
		$control_supplier_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing supplier_name - end
//	processing supplier_number - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_supplier_number = $pageObject->getControl("supplier_number", $id);
		$control_supplier_number->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing supplier_number - end
//	processing batch_number - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_batch_number = $pageObject->getControl("batch_number", $id);
		$control_batch_number->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing batch_number - end
//	processing batch_size - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_batch_size = $pageObject->getControl("batch_size", $id);
		$control_batch_size->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing batch_size - end
...
//	processing supplier_name - start
				...
				if($inlineAddOption)
				{
					$control_supplier_name->afterSuccessfulSave();
				}
//	processing supplier_name - end
//	processing supplier_number - start
				$inlineAddOption = true;
				if($inlineAddOption)
				{
					$control_supplier_number->afterSuccessfulSave();
				}
//	processing supplier_number - end
//	processing batch_number - start
				$inlineAddOption = true;
				if($inlineAddOption)
				{
					$control_batch_number->afterSuccessfulSave();
				}
//	processing batch_number - end
//	processing batch_size - start
				$inlineAddOption = true;
				if($inlineAddOption)
				{
					$control_batch_size->afterSuccessfulSave();
				}
//	processing batch_size - end
//	processing Date - start
				$inlineAddOption = true;
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("simcards", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("simcards", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("simcards", $pageObject->getPageType());
...
{
	$defvalues["Date"] = GetDefaultValue("Date", PAGE_ADD);
}
...
if($readavalues)
{
	$defvalues["supplier_name"]=@$avalues["supplier_name"];
	$defvalues["supplier_number"]=@$avalues["supplier_number"];
	$defvalues["batch_number"]=@$avalues["batch_number"];
	$defvalues["batch_size"]=@$avalues["batch_size"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("supplier_name"))
		$xt->assign("supplier_name_fieldblock",true);
	else
		$xt->assign("supplier_name_tabfieldblock",true);
	$xt->assign("supplier_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("supplier_name_label","<label for=\"".GetInputElementId("supplier_name", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("supplier_number"))
		$xt->assign("supplier_number_fieldblock",true);
	else
		$xt->assign("supplier_number_tabfieldblock",true);
	$xt->assign("supplier_number_label",true);
	if(isEnableSection508())
		$xt->assign_section("supplier_number_label","<label for=\"".GetInputElementId("supplier_number", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("batch_number"))
		$xt->assign("batch_number_fieldblock",true);
	else
		$xt->assign("batch_number_tabfieldblock",true);
	$xt->assign("batch_number_label",true);
	if(isEnableSection508())
		$xt->assign_section("batch_number_label","<label for=\"".GetInputElementId("batch_number", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("batch_size"))
		$xt->assign("batch_size_fieldblock",true);
	else
		$xt->assign("batch_size_tabfieldblock",true);
	$xt->assign("batch_size_label",true);
	if(isEnableSection508())
		$xt->assign_section("batch_size_label","<label for=\"".GetInputElementId("batch_size", $id, PAGE_ADD)."\">","</label>");
...
//	show readonly fields
	$pageObject->readOnlyFields["supplier_number"] = $pageObject->showDBValue("supplier_number", $defvalues);
	$pageObject->readOnlyFields["batch_size"] = $pageObject->showDBValue("batch_size", $defvalues);
$linkdata="";
...
//	supplier_name
	...
	if($display)
	{	
		$showValues["supplier_name"] = $pageObject->showDBValue("supplier_name", $data, $keylink);
		$showFields[] = "supplier_name";
	}	
//	supplier_number
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$showValues["supplier_number"] = $pageObject->showDBValue("supplier_number", $data, $keylink);
		$showFields[] = "supplier_number";
	}	
//	batch_number
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$showValues["batch_number"] = $pageObject->showDBValue("batch_number", $data, $keylink);
		$showFields[] = "batch_number";
	}	
//	batch_size
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$showValues["batch_size"] = $pageObject->showDBValue("batch_size", $data, $keylink);
		$showFields[] = "batch_size";
	}	
//	Date
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	if($display)
	{	
		$showValues["Date"] = $pageObject->showDBValue("Date", $data, $keylink);
		$showFields[] = "Date";
	}	
//	pos_number
	$display = false;
...
//	status
	...
	if($display)
	{	
		$showValues["status"] = $pageObject->showDBValue("status", $data, $keylink);
		$showFields[] = "status";
	}	
		$showRawValues["ID"] = substr($data["ID"],0,100);
		$showRawValues["supplier_name"] = substr($data["supplier_name"],0,100);
		$showRawValues["supplier_number"] = substr($data["supplier_number"],0,100);
		$showRawValues["batch_number"] = substr($data["batch_number"],0,100);
		$showRawValues["batch_size"] = substr($data["batch_size"],0,100);
