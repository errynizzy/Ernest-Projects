...
$layout = new TLayout("edit_basic_2col_center", "RoundedBlue_white", "MobileBlue_white");
$layout->version = 2;
...
$page_layouts["simcards_batches_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("simcards_batches", "inline_edit");
else 
	$templatefile = GetTemplateName("simcards_batches", "edit");
...
//	processing batch_name - begin
	$condition = 1;

	if($condition)
	{
		$control_batch_name = $pageObject->getControl("batch_name", $id);
		$control_batch_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing batch_name - end
//	processing ICCID - begin
	$condition = 1;

	if($condition)
	{
		$control_ICCID = $pageObject->getControl("ICCID", $id);
		$control_ICCID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_ICCID->getWebValue()!==false)
			$keys["ICCID"] = $control_ICCID->getWebValue();
	}
//	processing ICCID - end
...
//	processing batch_name - begin
				...
				if($condition)
				{
					$control_batch_name->afterSuccessfulSave();
				}
//	processing batch_name - end
//	processing ICCID - begin
				$condition = 1;
				...
				if($condition)
				{
					$control_ICCID->afterSuccessfulSave();
				}
//	processing ICCID - end
//	processing Status - begin
				$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("simcards_batches", $pageObject->getPageType(), $keyGetQ);
...
if($readevalues)
{
	$data["batch_name"] = $evalues["batch_name"];
	$data["ICCID"] = $evalues["ICCID"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("batch_name_label","<label for=\"".GetInputElementId("batch_name", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ICCID_label","<label for=\"".GetInputElementId("ICCID", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	...
	$showDetailKeys["activators"]["masterkey1"] = $data["Activator_Number"];		
	
	$showDetailKeys["returned_forms"]["masterkey1"] = $data["ICCID"];		
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("simcards_batches", "view", $viewlink)."'\"");
