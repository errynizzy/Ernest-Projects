...
$page_layouts["activators_add"] = $layout;
...
{
	$templatefile = GetTemplateName("activators", "inline_add");
}
else
{
	$templatefile = GetTemplateName("activators", "add");
}
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("activators", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("activators", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("activators", $pageObject->getPageType());
...
//	Manager_Name
	...
	if($display)
	{	
		$showValues["Manager_Name"] = $pageObject->showDBValue("Manager_Name", $data, $keylink);
...
//	Manager_Number
	...
	if($display)
	{	
		$showValues["Manager_Number"] = $pageObject->showDBValue("Manager_Number", $data, $keylink);
...
//	Team_Leader
	...
	if($display)
	{	
		$showValues["Team_Leader"] = $pageObject->showDBValue("Team_Leader", $data, $keylink);
...
//	last_chekin
	...
	if($display)
	{	
		$showValues["last_chekin"] = $pageObject->showDBValue("last_chekin", $data, $keylink);
