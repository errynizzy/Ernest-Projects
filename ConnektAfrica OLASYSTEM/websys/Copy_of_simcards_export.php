...
include("include/Copy_of_simcards_variables.php");
...
$page_layouts["Copy_of_simcards_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["supplier_name"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["supplier_name"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$pageObject->display(GetTemplateName("Copy_of_simcards", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Total"] = $pageObject->getExportValue("Total", $row);
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("Copy_of_simcards","pos_number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("Copy_of_simcards","Total")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("Copy_of_simcards","Active")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("Copy_of_simcards","supplier_name")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("Copy_of_simcards","pos_number")."</td>";
		echo "<td>".GetFieldLabel("Copy_of_simcards","Total")."</td>";
		echo "<td>".GetFieldLabel("Copy_of_simcards","Active")."</td>";
		echo "<td>".GetFieldLabel("Copy_of_simcards","supplier_name")."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["pos_number"] = $pageObject->getViewControl("pos_number")->getExportValue($row, "");
					$values["Total"] = $pageObject->getViewControl("Total")->getExportValue($row, "");
