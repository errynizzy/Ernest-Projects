...
$page_layouts["inout_export"] = $layout;
...
$pageObject->display(GetTemplateName("inout", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["CUST_NAME"] = $pageObject->getExportValue("CUST_NAME", $row);
			$values["ACC_NBR"] = $pageObject->getExportValue("ACC_NBR", $row);
			$values["PRE_BALANCE"] = $pageObject->getExportValue("PRE_BALANCE", $row);
			$values["CHARGE"] = $pageObject->getExportValue("CHARGE", $row);
			$values["TRANSFER_MODE"] = $pageObject->getExportValue("TRANSFER_MODE", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["CUST_NAME"] = $pageObject->getViewControl("CUST_NAME")->getExportValue($row, "");
			$values["ACC_NBR"] = $pageObject->getViewControl("ACC_NBR")->getExportValue($row, "");
			$values["PRE_BALANCE"] = $pageObject->getViewControl("PRE_BALANCE")->getExportValue($row, "");
			$values["CHARGE"] = $pageObject->getViewControl("CHARGE")->getExportValue($row, "");
			$values["TRANSFER_MODE"] = $pageObject->getViewControl("TRANSFER_MODE")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["CUST_NAME"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ACC_NBR"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["PRE_BALANCE"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["CHARGE"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["TRANSFER_MODE"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","EVENT_DATE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","CUST_NAME")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","ACC_NBR")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","PRE_BALANCE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","CHARGE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("inout","TRANSFER_MODE")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("inout","EVENT_DATE")."</td>";
		echo "<td>".GetFieldLabel("inout","CUST_NAME")."</td>";
		echo "<td>".GetFieldLabel("inout","ACC_NBR")."</td>";
		echo "<td>".GetFieldLabel("inout","PRE_BALANCE")."</td>";
		echo "<td>".GetFieldLabel("inout","CHARGE")."</td>";
		echo "<td>".GetFieldLabel("inout","TRANSFER_MODE")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["CUST_NAME"]);
					else
						echo $values["CUST_NAME"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ACC_NBR"]);
					else
						echo $values["ACC_NBR"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["PRE_BALANCE"]);
					else
						echo $values["PRE_BALANCE"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["TRANSFER_MODE"]);
					else
						echo $values["TRANSFER_MODE"];
			echo '</td>';
			echo "</tr>";
		}
