...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="Activations and Airtime Transfers" && $field=="Percent")
	{
		$value = round((($data["With_Airtime"] / $data["Total_Activations"]) * 100), 2)  .'%';
;
		return $value;
	}	
				if($table=="Registration Forms Outstanding  " && $field=="Percent")
	{
		$value = round((($data["Returned"] / $data["Activations"]) * 100), 2)  .'%';
;
		return $value;
	}	
				if($table=="Activations per active FL" && $field=="Percent")
	{
		$value = round((($data["With_Airtime"] / $data["Activations"]) * 100), 2)  .'%';
;
		return $value;
	}	
	return $value;
