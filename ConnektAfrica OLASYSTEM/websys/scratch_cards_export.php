...
$page_layouts["scratch_cards_export"] = $layout;
...
$pageObject->display(GetTemplateName("scratch_cards", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["RECHARGE_DATE"] = $pageObject->getExportValue("RECHARGE_DATE", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["RECHARGE_DATE"] = $pageObject->getViewControl("RECHARGE_DATE")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["RECHARGE_DATE"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("scratch_cards","SERIAL_NO")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("scratch_cards","ACC_NBR")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("scratch_cards","DENOMINATION")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("scratch_cards","RECHARGE_DATE")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("scratch_cards","SERIAL_NO")."</td>";
		echo "<td>".GetFieldLabel("scratch_cards","ACC_NBR")."</td>";
		echo "<td>".GetFieldLabel("scratch_cards","DENOMINATION")."</td>";
		echo "<td>".GetFieldLabel("scratch_cards","RECHARGE_DATE")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["SERIAL_NO"]);
					else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DENOMINATION"]);
					else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["RECHARGE_DATE"]);
					else
						echo $values["RECHARGE_DATE"];
			echo '</td>';
			echo "</tr>";
		}
