...
$page_layouts["activated_sims_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("activated_sims", "list", "a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Subscriber_ICCID";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Subscriber_ICCID");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Subscriber_Number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Subscriber_Number");
$fieldsArr[] = $arr;
$arr = array();
...
while($data)
{
	...
	$value = $pageObject->showDBValue("Subscriber_ICCID", $data, $keylink);
	if( $mainTableOwnerID == "Subscriber_ICCID" )
		$ownerIdValue = $value;
	$xt->assign("Subscriber_ICCID_value",$value);
	if(!$pageObject->isAppearOnTabs("Subscriber_ICCID"))
		$xt->assign("Subscriber_ICCID_fieldblock",true);
	else
		$xt->assign("Subscriber_ICCID_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Subscriber_ICCID");
	$value = $pageObject->showDBValue("Subscriber_Number", $data, $keylink);
	if( $mainTableOwnerID == "Subscriber_Number" )
		$ownerIdValue = $value;
	$xt->assign("Subscriber_Number_value",$value);
	if(!$pageObject->isAppearOnTabs("Subscriber_Number"))
		$xt->assign("Subscriber_Number_fieldblock",true);
	else
		$xt->assign("Subscriber_Number_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Subscriber_Number");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "activated_sims";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("activated_sims", "edit", $editlink)."'\"");
