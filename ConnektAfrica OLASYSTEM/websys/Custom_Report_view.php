...
$page_layouts["Custom_Report_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("Custom_Report", "list", "a=return");
	exit();
}
...
$fieldsArr = array();
$arr = array();
$arr['fName'] = "supplier_name";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("supplier_name");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "supplier_number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("supplier_number");
$fieldsArr[] = $arr;
$arr = array();
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Activation_Status";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Activation_Status");
$fieldsArr[] = $arr;
$arr = array();
...
while($data)
{
	...
	$value = $pageObject->showDBValue("supplier_name", $data, $keylink);
	if( $mainTableOwnerID == "supplier_name" )
		$ownerIdValue = $value;
	$xt->assign("supplier_name_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("supplier_name");
	$value = $pageObject->showDBValue("supplier_number", $data, $keylink);
	if( $mainTableOwnerID == "supplier_number" )
		$ownerIdValue = $value;
	$xt->assign("supplier_number_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("supplier_number");
	...
	if( $mainTableOwnerID == "Activation_Status" )
		$ownerIdValue = $value;
	$xt->assign("Activation_Status_value",$value);
	if(!$pageObject->isAppearOnTabs("Activation_Status"))
		$xt->assign("Activation_Status_fieldblock",true);
	else
		$xt->assign("Activation_Status_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Activation_Status");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "Custom Report";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("Custom_Report", "edit", $editlink)."'\"");
