...
$page_layouts["controller_add"] = $layout;
...
{
	$templatefile = GetTemplateName("controller", "inline_add");
}
else
{
	$templatefile = GetTemplateName("controller", "add");
}
...
//	processing Sales_Coordinator_Number - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Sales_Coordinator_Number = $pageObject->getControl("Sales_Coordinator_Number", $id);
		$control_Sales_Coordinator_Number->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Team_Leader_Number - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Team_Leader_Number = $pageObject->getControl("Team_Leader_Number", $id);
		$control_Team_Leader_Number->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("controller", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("controller", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("controller", $pageObject->getPageType());
...
if($readavalues)
{
	...
	$defvalues["Sales_Coordinator_Number"]=@$avalues["Sales_Coordinator_Number"];
	$defvalues["Team_Leader_Name"]=@$avalues["Team_Leader_Name"];
	$defvalues["Team_Leader_Number"]=@$avalues["Team_Leader_Number"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Sales_Coordinator_Number_label","<label for=\"".GetInputElementId("Sales_Coordinator_Number", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Team_Leader_Number_label","<label for=\"".GetInputElementId("Team_Leader_Number", $id, PAGE_ADD)."\">","</label>");
...
//	Sales_Coordinator_Number
	...
	if($display)
	{	
		$showValues["Sales_Coordinator_Number"] = $pageObject->showDBValue("Sales_Coordinator_Number", $data, $keylink);
...
//	Team_Leader_Number
	...
	if($display)
	{	
		$showValues["Team_Leader_Number"] = $pageObject->showDBValue("Team_Leader_Number", $data, $keylink);
