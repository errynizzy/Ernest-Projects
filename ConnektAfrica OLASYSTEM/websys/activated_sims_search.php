...
include("include/activated_sims_variables.php");
...
$page_layouts["activated_sims_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = GetTemplateName("activated_sims", "search");
...
$params['shortTableName'] = 'activated_sims';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("Subscriber_ICCID"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Subscriber_ICCID")] = GetTableURL($pageObject->pSet->getLookupTable("Subscriber_ICCID"));
	
	$pageObject->fillFieldToolTips("Subscriber_ICCID");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Subscriber_ICCID");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Subscriber_ICCID";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = $pageObject->pSet->getDefaultValue( "Subscriber_ICCID" );
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
		$firstFieldParams['not'] = false;
	}
	...
	if(isEnableSection508())
		$xt->assign_section("Subscriber_ICCID_label","<label for=\"".GetInputElementId("Subscriber_ICCID", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Subscriber_ICCID_label", true);
	
	$xt->assign("Subscriber_ICCID_fieldblock", true);
	$xt->assignbyref("Subscriber_ICCID_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Subscriber_ICCID_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Subscriber_ICCID_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Subscriber_ICCID", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Subscriber_ICCID");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Subscriber_ICCID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Subscriber_ICCID", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("Subscriber_Number"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("Subscriber_Number")] = GetTableURL($pageObject->pSet->getLookupTable("Subscriber_Number"));
	
	$pageObject->fillFieldToolTips("Subscriber_Number");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Subscriber_Number");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Subscriber_Number";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = $pageObject->pSet->getDefaultValue( "Subscriber_Number" );
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
		$firstFieldParams['not'] = false;
	}
	...
	if(isEnableSection508())
		$xt->assign_section("Subscriber_Number_label","<label for=\"".GetInputElementId("Subscriber_Number", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("Subscriber_Number_label", true);
	
	$xt->assign("Subscriber_Number_fieldblock", true);
	$xt->assignbyref("Subscriber_Number_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("Subscriber_Number_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("Subscriber_Number_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_Subscriber_Number", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Subscriber_Number");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Subscriber_Number", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Subscriber_Number", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
