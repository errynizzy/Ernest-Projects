...
$page_layouts["returned_forms_export"] = $layout;
...
$pageObject->display(GetTemplateName("returned_forms", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["MSISDN"] = $pageObject->getExportValue("MSISDN", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["MSISDN"] = $pageObject->getViewControl("MSISDN")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["MSISDN"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("returned_forms","ID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("returned_forms","ICCID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("returned_forms","MSISDN")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("returned_forms","Status")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("returned_forms","Date")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("returned_forms","ID")."</td>";
		echo "<td>".GetFieldLabel("returned_forms","ICCID")."</td>";
		echo "<td>".GetFieldLabel("returned_forms","MSISDN")."</td>";
		echo "<td>".GetFieldLabel("returned_forms","Status")."</td>";
		echo "<td>".GetFieldLabel("returned_forms","Date")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["MSISDN"]);
					else
						echo $values["MSISDN"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
