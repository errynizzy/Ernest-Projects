...
$page_layouts["scratch_cards_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("scratch_cards", "list", "a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "RECHARGE_DATE";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("RECHARGE_DATE");
$fieldsArr[] = $arr;
...
while($data)
{
	...
	if( $mainTableOwnerID == "ACC_NBR" )
		$ownerIdValue = $value;
	$xt->assign("ACC_NBR_value",$value);
	...
	$value = $pageObject->showDBValue("RECHARGE_DATE", $data, $keylink);
	if( $mainTableOwnerID == "RECHARGE_DATE" )
		$ownerIdValue = $value;
	$xt->assign("RECHARGE_DATE_value",$value);
	if(!$pageObject->isAppearOnTabs("RECHARGE_DATE"))
		$xt->assign("RECHARGE_DATE_fieldblock",true);
	else
		$xt->assign("RECHARGE_DATE_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("RECHARGE_DATE");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "scratch_cards";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("scratch_cards", "edit", $editlink)."'\"");
