...
include("include/controller_variables.php");
...
$page_layouts["controller_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("controller", "inline_edit");
else 
	$templatefile = GetTemplateName("controller", "edit");
...
//	processing Sales_Coordinator_Name - begin
	$condition = 1;

	if($condition)
	{
		$control_Sales_Coordinator_Name = $pageObject->getControl("Sales_Coordinator_Name", $id);
		$control_Sales_Coordinator_Name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Sales_Coordinator_Name - end
//	processing Sales_Coordinator_Number - begin
	$condition = 1;

	if($condition)
	{
		$control_Sales_Coordinator_Number = $pageObject->getControl("Sales_Coordinator_Number", $id);
		$control_Sales_Coordinator_Number->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Sales_Coordinator_Number - end
//	processing Team_Leader_Name - begin
	$condition = 1;

	if($condition)
	{
		$control_Team_Leader_Name = $pageObject->getControl("Team_Leader_Name", $id);
		$control_Team_Leader_Name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Team_Leader_Name - end
//	processing Team_Leader_Number - begin
	$condition = 1;

	if($condition)
	{
		$control_Team_Leader_Number = $pageObject->getControl("Team_Leader_Number", $id);
		$control_Team_Leader_Number->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Team_Leader_Number - end
...
//	processing Sales_Coordinator_Number - begin
				...
				if($condition)
				{
					$control_Sales_Coordinator_Number->afterSuccessfulSave();
				}
//	processing Sales_Coordinator_Number - end
//	processing Team_Leader_Name - begin
				$condition = 1;
...
//	processing Team_Leader_Number - begin
				...
				if($condition)
				{
					$control_Team_Leader_Number->afterSuccessfulSave();
				}
//	processing Team_Leader_Number - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("controller", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("controller", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Sales_Coordinator_Name"] = $evalues["Sales_Coordinator_Name"];
	$data["Sales_Coordinator_Number"] = $evalues["Sales_Coordinator_Number"];
	$data["Team_Leader_Name"] = $evalues["Team_Leader_Name"];
	$data["Team_Leader_Number"] = $evalues["Team_Leader_Number"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Sales_Coordinator_Name_label","<label for=\"".GetInputElementId("Sales_Coordinator_Name", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Sales_Coordinator_Number"))
		$xt->assign("Sales_Coordinator_Number_fieldblock",true);
	else
		$xt->assign("Sales_Coordinator_Number_tabfieldblock",true);
	$xt->assign("Sales_Coordinator_Number_label",true);
	if(isEnableSection508())
		$xt->assign_section("Sales_Coordinator_Number_label","<label for=\"".GetInputElementId("Sales_Coordinator_Number", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Team_Leader_Name_label","<label for=\"".GetInputElementId("Team_Leader_Name", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("Team_Leader_Number"))
		$xt->assign("Team_Leader_Number_fieldblock",true);
	else
		$xt->assign("Team_Leader_Number_tabfieldblock",true);
	$xt->assign("Team_Leader_Number_label",true);
	if(isEnableSection508())
		$xt->assign_section("Team_Leader_Number_label","<label for=\"".GetInputElementId("Team_Leader_Number", $id, PAGE_EDIT)."\">","</label>");
...
//	Sales_Coordinator_Name - 
	$value = $pageObject->showDBValue("Sales_Coordinator_Name", $data, $keylink);
	$showValues["Sales_Coordinator_Name"] = $value;
...
//	Sales_Coordinator_Number - 
	$value = $pageObject->showDBValue("Sales_Coordinator_Number", $data, $keylink);
	$showValues["Sales_Coordinator_Number"] = $value;
	$showFields[] = "Sales_Coordinator_Number";
		$showRawValues["Sales_Coordinator_Number"] = substr($data["Sales_Coordinator_Number"],0,100);

//	Team_Leader_Name - 
	$value = $pageObject->showDBValue("Team_Leader_Name", $data, $keylink);
	$showValues["Team_Leader_Name"] = $value;
	$showFields[] = "Team_Leader_Name";
		$showRawValues["Team_Leader_Name"] = substr($data["Team_Leader_Name"],0,100);

//	Team_Leader_Number - 
	$value = $pageObject->showDBValue("Team_Leader_Number", $data, $keylink);
	$showValues["Team_Leader_Number"] = $value;
	$showFields[] = "Team_Leader_Number";
		$showRawValues["Team_Leader_Number"] = substr($data["Team_Leader_Number"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "controller";
			$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("controller", "view", $viewlink)."'\"");
