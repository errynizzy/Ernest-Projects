...
$layout = new TLayout("search", "RoundedBlue_white", "MobileBlue_white");
$layout->version = 2;
...
$page_layouts["returned_forms_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = GetTemplateName("returned_forms", "search");
...
$params['shortTableName'] = 'returned_forms';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("MSISDN"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("MSISDN")] = GetTableURL($pageObject->pSet->getLookupTable("MSISDN"));
	
	$pageObject->fillFieldToolTips("MSISDN");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("MSISDN");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "MSISDN";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = $pageObject->pSet->getDefaultValue( "MSISDN" );
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
		$firstFieldParams['not'] = false;
	}
	...
	if(isEnableSection508())
		$xt->assign_section("MSISDN_label","<label for=\"".GetInputElementId("MSISDN", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("MSISDN_label", true);
	
	$xt->assign("MSISDN_fieldblock", true);
	$xt->assignbyref("MSISDN_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("MSISDN_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("MSISDN_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_MSISDN", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("MSISDN");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MSISDN", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"MSISDN", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
