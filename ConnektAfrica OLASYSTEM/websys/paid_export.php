...
$page_layouts["paid_export"] = $layout;
...
$pageObject->display(GetTemplateName("paid", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	{
		$values = array();
			$values["Paid_MSISDN"] = $pageObject->getExportValue("Paid_MSISDN", $row);
			$values["Date_Paid"] = $pageObject->getExportValue("Date_Paid", $row);
...
// write data rows
	...
	{
		$values = array();
			$values["Paid_MSISDN"] = $pageObject->getViewControl("Paid_MSISDN")->getExportValue($row, "");
			$values["Date_Paid"] = $pageObject->getViewControl("Date_Paid")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Paid_MSISDN"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Date_Paid"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("paid","Paid_MSISDN")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("paid","Date_Paid")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("paid","Paid_MSISDN")."</td>";
		echo "<td>".GetFieldLabel("paid","Date_Paid")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Paid_MSISDN"]);
					else
						echo $values["Paid_MSISDN"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Date_Paid"]);
					else
						echo $values["Date_Paid"];
			echo '</td>';
			echo "</tr>";
		}
