...
$page_layouts["form_tracking_view"] = $layout;
...
$keys = array();
$keys["D_SL_DSO_RDO"] = postvalue("editid1");
...
if (!sizeof($data)) 
{
	HeaderRedirect("form_tracking", "list", "a=return");
	exit();
}
...
$fieldsArr = array();
$arr = array();
$arr['fName'] = "D_SL_DSO_RDO";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("D_SL_DSO_RDO");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "TOTAL_SIMCARDS_GIVEN";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("TOTAL_SIMCARDS_GIVEN");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "SIMCARDS_ACTIVATED";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("SIMCARDS_ACTIVATED");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "SIMCARDS_RETURNED";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("SIMCARDS_RETURNED");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "SIMCARDS_REJECTED";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("SIMCARDS_REJECTED");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "SIMCARDS_NOT_RETURNED";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("SIMCARDS_NOT_RETURNED");
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", $pageObject->showDBValue("D_SL_DSO_RDO", $data));

	$keylink = "";
	$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["D_SL_DSO_RDO"]));

	$value = $pageObject->showDBValue("D_SL_DSO_RDO", $data, $keylink);
	if( $mainTableOwnerID == "D_SL_DSO_RDO" )
		$ownerIdValue = $value;
	$xt->assign("D_SL_DSO_RDO_value",$value);
	if(!$pageObject->isAppearOnTabs("D_SL_DSO_RDO"))
		$xt->assign("D_SL_DSO_RDO_fieldblock",true);
	else
		$xt->assign("D_SL_DSO_RDO_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("D_SL_DSO_RDO");
	$value = $pageObject->showDBValue("TOTAL_SIMCARDS_GIVEN", $data, $keylink);
	if( $mainTableOwnerID == "TOTAL_SIMCARDS_GIVEN" )
		$ownerIdValue = $value;
	$xt->assign("TOTAL_SIMCARDS_GIVEN_value",$value);
	if(!$pageObject->isAppearOnTabs("TOTAL_SIMCARDS_GIVEN"))
		$xt->assign("TOTAL_SIMCARDS_GIVEN_fieldblock",true);
	else
		$xt->assign("TOTAL_SIMCARDS_GIVEN_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("TOTAL_SIMCARDS_GIVEN");
	$value = $pageObject->showDBValue("SIMCARDS_ACTIVATED", $data, $keylink);
	if( $mainTableOwnerID == "SIMCARDS_ACTIVATED" )
		$ownerIdValue = $value;
	$xt->assign("SIMCARDS_ACTIVATED_value",$value);
	if(!$pageObject->isAppearOnTabs("SIMCARDS_ACTIVATED"))
		$xt->assign("SIMCARDS_ACTIVATED_fieldblock",true);
	else
		$xt->assign("SIMCARDS_ACTIVATED_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("SIMCARDS_ACTIVATED");
	$value = $pageObject->showDBValue("SIMCARDS_RETURNED", $data, $keylink);
	if( $mainTableOwnerID == "SIMCARDS_RETURNED" )
		$ownerIdValue = $value;
	$xt->assign("SIMCARDS_RETURNED_value",$value);
	if(!$pageObject->isAppearOnTabs("SIMCARDS_RETURNED"))
		$xt->assign("SIMCARDS_RETURNED_fieldblock",true);
	else
		$xt->assign("SIMCARDS_RETURNED_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("SIMCARDS_RETURNED");
	$value = $pageObject->showDBValue("SIMCARDS_REJECTED", $data, $keylink);
	if( $mainTableOwnerID == "SIMCARDS_REJECTED" )
		$ownerIdValue = $value;
	$xt->assign("SIMCARDS_REJECTED_value",$value);
	if(!$pageObject->isAppearOnTabs("SIMCARDS_REJECTED"))
		$xt->assign("SIMCARDS_REJECTED_fieldblock",true);
	else
		$xt->assign("SIMCARDS_REJECTED_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("SIMCARDS_REJECTED");
	$value = $pageObject->showDBValue("SIMCARDS_NOT_RETURNED", $data, $keylink);
	if( $mainTableOwnerID == "SIMCARDS_NOT_RETURNED" )
		$ownerIdValue = $value;
	$xt->assign("SIMCARDS_NOT_RETURNED_value",$value);
	if(!$pageObject->isAppearOnTabs("SIMCARDS_NOT_RETURNED"))
		$xt->assign("SIMCARDS_NOT_RETURNED_fieldblock",true);
	else
		$xt->assign("SIMCARDS_NOT_RETURNED_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("SIMCARDS_NOT_RETURNED");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "form_tracking";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("form_tracking", "edit", $editlink)."'\"");
