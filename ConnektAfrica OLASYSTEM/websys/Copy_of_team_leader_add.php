...
$page_layouts["Copy_of_team_leader_add"] = $layout;
...
{
	$templatefile = GetTemplateName("Copy_of_team_leader", "inline_add");
}
else
{
	$templatefile = GetTemplateName("Copy_of_team_leader", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("Copy_of_team_leader", "add");
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("Copy_of_team_leader", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("Copy_of_team_leader", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("Copy_of_team_leader", $pageObject->getPageType());
...
//	Mng
	...
	if($display)
	{	
		$showValues["Mng"] = $pageObject->showDBValue("Mng", $data, $keylink);
		$showFields[] = "Mng";
	}	
		...
		$showRawValues["Mng"] = substr($data["Mng"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of team_leader";
		$options['firstTime'] = 1;
	...
	$strTableName = "Copy of team_leader";
}
/////////////////////////////////////////////////////////////
