...
class RightsPage extends ListPage  
{
	...
	**/
	...
	function updateTablePermissions( $table, $group, $mask )
	{
		...
		$sql = "select  from  where =".$group." and =".db_prepare_string($table);
		...
		if($data)
		{
			...
			if( strlen($mask) )
			{
				$sql = "update  set ='".$mask."' where =".$group." and =".db_prepare_string($table);
			}
			else
			{
				$sql = "delete from  where =".$group." and =".db_prepare_string($table);
			}
		}
		else
		{
			if( !strlen($mask) )
				return;
			$sql = "insert into  (, , ) values (".$group.", ".db_prepare_string($table).", '".$mask."')";
		}
		$rs = db_exec( $sql, $conn);
	}
}
