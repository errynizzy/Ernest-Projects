...
class Chart
{
	...
	function Chart(&$ch_array, $param)
	{
		...
		if(count($detailKeysByM)) 
		{
			for($i=0;$i<count($detailKeysByM);$i++) 
			{
				...
				if(strlen($mValue) != 0)
					$masterWhere.= GetFullFieldName($detailKeysByM[$i], "", false)."=".$mValue;
				else 
					$masterWhere.= "1=0";
			}
		}
		...
		{	
			if( !$this->chartPreview && isset($_SESSION[$this->sessionPrefix.'_advsearch']) )
			{
				...
				$whereComponents = RunnerPage::sGetWhereComponents( $gQuery, $this->pSet, $searchClauseObj, $editControls );
	...
	function write_axes_custom()
	{
		...
		$scroll="false";
		if($this->chrt_array["appearance"]["cscroll"]=="true" && $this->totalRecords>$this->numRecordsToShow && $useFlashChartLibrary)
			$scroll="true";
	...
	function get_data($refr)
	{
		...
		if($this->cipherer != null)
			$row = $this->cipherer->DecryptFetchedArray($rs);
		else 
