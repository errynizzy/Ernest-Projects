...
class LoginPage extends RunnerPage 
{
	...
	*/
	function LogIn($pUsername,$pPassword){
		...
		$fieldList = "";
		$lSet = new ProjectSettings("users", PAGE_LIST);
		if($lSet->GetTableData(".sqlquery"))
			$fieldList = $lSet->GetTableData(".sqlquery")->toSql();
		if($fieldList)
		{
			if(!$this->pSet->isCaseInsensitiveUsername()) {
				$where = GetFullFieldName($cUserNameField,"users",false).
				   "=".$strUsername." and ".GetFullFieldName($cPasswordField,"users",false)."=".$strPassword;
   			} else {
				$where = db_upper(getFullFieldName($cUserNameField,"users",false)).
		...
		{
			$strSQL = "select * from `users` where ".AddFieldWrappers($cUserNameField)."=".$strUsername." and ".AddFieldWrappers($cPasswordField)."=".$strPassword;
					}
	 ...
	 *
	 */
	function Logout($page = "", $redirect = true){
		if($this->auditObj)
