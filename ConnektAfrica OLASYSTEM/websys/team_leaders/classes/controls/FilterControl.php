...
class FilterControl
{
	...
	{
		return $this->pSet->getFullFieldNameCiph($fName, $this->cipherer);
	}
	...
	{
		$caseStatement = "case when ".$condition." then ".$trueValue." else ".$falseValue." end";
	...
	/**
	 * A stab for not dependant filters
	 * @param String
	 * @return Array
	 */
	...
	protected function getControlHTML($value, $showValue, $dataValue, $totalValue, $separator, $parentFiltersData = null) 
	{
		...
		{
			$href = GetTableLink($this->tName, 'list', 'f=('.$this->fName.$separator.$encodeDataValue.')');
			$label = '<a href="'.$href.'" data-filtervalue="'.rawurlencode($encodeDataValue).'" '.$extraDataAttrs.' class="'.$this->gfName.'-filter-value">'.$showValue.'</a>';
		} 
		else
		{
