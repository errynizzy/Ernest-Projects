...
function checkTableName($shortTName, $type=false)
{
	...
	if ("team_leader" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("team_leader");
...
function GetUserPermissionsStatic($table="")
{
	...
	if($table == "team_leader" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "team_leader" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	// default permissions	
	if($table == "team_leader")
	{
		return "AEDSPI".$extraPerm;
	}
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password)
{
	...
	$_SESSION["_team_leader_OwnerID"] = $data["ID"];
