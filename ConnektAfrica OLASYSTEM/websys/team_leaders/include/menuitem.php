...
{	
	...
	{			
		// for all menuItems
		foreach($menuNodes as $i=>$node)
		{
			// if this element of array is child
			if ($node["parent"] == $menuParent->id)
			{
				// create new obj
				$menuChild = new MenuItem($node, $menuNodes, $menuParent);				
				// adds to parent
			...
			{
				if (!isset($this->pageTypesInMenuForThisTable[$node['pageType']])&&$this->table==$node['table'])
				{					
					$this->pageTypesInMenuForThisTable[$node['pageType']]['count'] = 1;
					$this->pageTypesInMenuForThisTable[$node['pageType']]['ids'] = array('0'=>$node['id']);						
				}
				elseif ($this->table==$node['table'])
				{
					$this->pageTypesInMenuForThisTable[$node['pageType']]['count']++;
					$this->pageTypesInMenuForThisTable[$node['pageType']]['ids'][] = $node['id'];
				}
			}					
		}
	}		
