...
$page_layouts["Copy_of_batches_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("Copy_of_batches", "inline_edit");
else 
	$templatefile = GetTemplateName("Copy_of_batches", "edit");
...
//	processing Batch_Number - begin
	$condition = 1;

	if($condition)
	{
		$control_Batch_Number = $pageObject->getControl("Batch_Number", $id);
		$control_Batch_Number->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Batch_Number - end
//	processing Given_Batch_Number - begin
	$condition = 1;

	if($condition)
	{
		$control_Given_Batch_Number = $pageObject->getControl("Given_Batch_Number", $id);
		$control_Given_Batch_Number->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing Given_Batch_Number - end
//	processing Batch_size - begin
	$condition = 1;

	if($condition)
	{
		$control_Batch_size = $pageObject->getControl("Batch_size", $id);
		$control_Batch_size->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("Copy_of_batches", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("Copy_of_batches", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["Batch_Number"] = $evalues["Batch_Number"];
	$data["Given_Batch_Number"] = $evalues["Given_Batch_Number"];
	$data["Batch_size"] = $evalues["Batch_size"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Batch_Number_label","<label for=\"".GetInputElementId("Batch_Number", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Given_Batch_Number_label","<label for=\"".GetInputElementId("Given_Batch_Number", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Batch_size_label","<label for=\"".GetInputElementId("Batch_size", $id, PAGE_EDIT)."\">","</label>");
...
//	Batch_Number - 
	$value = $pageObject->showDBValue("Batch_Number", $data, $keylink);
	$showValues["Batch_Number"] = $value;
	$showFields[] = "Batch_Number";
		$showRawValues["Batch_Number"] = substr($data["Batch_Number"],0,100);

//	Given_Batch_Number - 
	$value = $pageObject->showDBValue("Given_Batch_Number", $data, $keylink);
	$showValues["Given_Batch_Number"] = $value;
	$showFields[] = "Given_Batch_Number";
		$showRawValues["Given_Batch_Number"] = substr($data["Given_Batch_Number"],0,100);

//	Batch_size - 
	$value = $pageObject->showDBValue("Batch_size", $data, $keylink);
	$showValues["Batch_size"] = $value;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("Copy_of_batches", "view", $viewlink)."'\"");
