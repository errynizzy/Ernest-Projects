...
$page_layouts["simcards_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("simcards", "list", "a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "pos_number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("pos_number");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "status";
...
while($data)
{
	...
	if( $mainTableOwnerID == "batch_number" )
		$ownerIdValue = $value;
	$xt->assign("batch_number_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("batch_number");
	$value = $pageObject->showDBValue("batch_size", $data, $keylink);
	if( $mainTableOwnerID == "batch_size" )
		$ownerIdValue = $value;
	$xt->assign("batch_size_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("batch_size");
	...
	if( $mainTableOwnerID == "pos_number" )
		$ownerIdValue = $value;
	$xt->assign("pos_number_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("pos_number");
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("simcards", "edit", $editlink)."'\"");
