...
$page_layouts["Copy_of_batches_add"] = $layout;
...
{
	$templatefile = GetTemplateName("Copy_of_batches", "inline_add");
}
else
{
	$templatefile = GetTemplateName("Copy_of_batches", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("Copy_of_batches", "add");
...
//	processing Given_Batch_Number - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Given_Batch_Number = $pageObject->getControl("Given_Batch_Number", $id);
		$control_Given_Batch_Number->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Given_Batch_Number - end
...
//	processing Given_Batch_Number - start
				...
				if($inlineAddOption)
				{
					$control_Given_Batch_Number->afterSuccessfulSave();
				}
//	processing Given_Batch_Number - end
//	processing Batch_size - start
				$inlineAddOption = true;
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("Copy_of_batches", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("Copy_of_batches", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("Copy_of_batches", $pageObject->getPageType());
...
if($readavalues)
{
	...
	$defvalues["Given_Batch_Number"]=@$avalues["Given_Batch_Number"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("Given_Batch_Number"))
		$xt->assign("Given_Batch_Number_fieldblock",true);
	else
		$xt->assign("Given_Batch_Number_tabfieldblock",true);
	$xt->assign("Given_Batch_Number_label",true);
	if(isEnableSection508())
		$xt->assign_section("Given_Batch_Number_label","<label for=\"".GetInputElementId("Given_Batch_Number", $id, PAGE_ADD)."\">","</label>");
...
//	Given_Batch_Number
	...
	if($display)
	{	
		$showValues["Given_Batch_Number"] = $pageObject->showDBValue("Given_Batch_Number", $data, $keylink);
		$showFields[] = "Given_Batch_Number";
	}	
//	Batch_size
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	Status
	...
	{	
		...
		$showRawValues["Given_Batch_Number"] = substr($data["Given_Batch_Number"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "Copy of batches";
		$options['firstTime'] = 1;
