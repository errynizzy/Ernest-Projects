...
{
	$templatefile = GetTemplateName("inout", "inline_add");
}
else
{
	$templatefile = GetTemplateName("inout", "add");
}
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing ACCT_BOOK_ID - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ACCT_BOOK_ID = $pageObject->getControl("ACCT_BOOK_ID", $id);
		$control_ACCT_BOOK_ID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing CUST_NAME - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CUST_NAME = $pageObject->getControl("CUST_NAME", $id);
		$control_CUST_NAME->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CUST_NAME - end
//	processing ACC_NBR - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_ACC_NBR = $pageObject->getControl("ACC_NBR", $id);
		$control_ACC_NBR->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing ACC_NBR - end
//	processing PRE_BALANCE - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_PRE_BALANCE = $pageObject->getControl("PRE_BALANCE", $id);
		$control_PRE_BALANCE->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing PRE_BALANCE - end
//	processing CHARGE - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_CHARGE = $pageObject->getControl("CHARGE", $id);
		$control_CHARGE->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CHARGE - end
//	processing TRANSFER_MODE - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_TRANSFER_MODE = $pageObject->getControl("TRANSFER_MODE", $id);
		$control_TRANSFER_MODE->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("inout", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("inout", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("inout", $pageObject->getPageType());
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["ACCT_BOOK_ID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["ACCT_BOOK_ID"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["ACCT_BOOK_ID"]=@$avalues["ACCT_BOOK_ID"];
	$defvalues["EVENT_DATE"]=@$avalues["EVENT_DATE"];
	$defvalues["CUST_NAME"]=@$avalues["CUST_NAME"];
	$defvalues["ACC_NBR"]=@$avalues["ACC_NBR"];
	$defvalues["PRE_BALANCE"]=@$avalues["PRE_BALANCE"];
	$defvalues["CHARGE"]=@$avalues["CHARGE"];
	$defvalues["TRANSFER_MODE"]=@$avalues["TRANSFER_MODE"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ACCT_BOOK_ID_label","<label for=\"".GetInputElementId("ACCT_BOOK_ID", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("CUST_NAME_label","<label for=\"".GetInputElementId("CUST_NAME", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ACC_NBR_label","<label for=\"".GetInputElementId("ACC_NBR", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("PRE_BALANCE_label","<label for=\"".GetInputElementId("PRE_BALANCE", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("CHARGE_label","<label for=\"".GetInputElementId("CHARGE", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("TRANSFER_MODE_label","<label for=\"".GetInputElementId("TRANSFER_MODE", $id, PAGE_ADD)."\">","</label>");
...
//	ACCT_BOOK_ID
	...
	if($display)
	{	
		$showValues["ACCT_BOOK_ID"] = $pageObject->showDBValue("ACCT_BOOK_ID", $data, $keylink);
...
//	CUST_NAME
	...
	if($display)
	{	
		$showValues["CUST_NAME"] = $pageObject->showDBValue("CUST_NAME", $data, $keylink);
...
//	ACC_NBR
	...
	if($display)
	{	
		$showValues["ACC_NBR"] = $pageObject->showDBValue("ACC_NBR", $data, $keylink);
		$showFields[] = "ACC_NBR";
	}	
//	PRE_BALANCE
	$display = false;
	...
	if($display)
	{	
		$showValues["PRE_BALANCE"] = $pageObject->showDBValue("PRE_BALANCE", $data, $keylink);
...
//	CHARGE
	...
	if($display)
	{	
		$showValues["CHARGE"] = $pageObject->showDBValue("CHARGE", $data, $keylink);
		$showFields[] = "CHARGE";
	}	
//	TRANSFER_MODE
	$display = false;
	...
	if($display)
	{	
		$showValues["TRANSFER_MODE"] = $pageObject->showDBValue("TRANSFER_MODE", $data, $keylink);
