...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="Efficiency" && $field=="Planned_Visits" && $ptype=="view")
	{
		$value = round((($data["Actual_Visits"] / $data["Planned_Visits"]) * 100), 2)  .'%';;
		return $value;
	}	
				if($table=="Efficiency" && $field=="Planned_Visits" && $ptype=="export")
	{
		$value = round((($data["Actual_Visits"] / $data["Planned_Visits"]) * 100), 2)  .'%';;
		return $value;
	}	
				if($table=="Efficiency" && $field=="Percent")
	{
		$value = round((($data["Actual_Visits"] / $data["Planned_Visits"]) * 100), 2)  .'%';;
		return $value;
	}	
				if($table=="Effectiveness" && $field=="Percent")
	{
		$value = round((($data["MY_Sales"] / $data["Total_Visits"]) * 100), 2)  .'%';;
		return $value;
	}	
	return $value;
}
