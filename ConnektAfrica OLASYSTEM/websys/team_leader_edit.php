...
$page_layouts["team_leader_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("team_leader", "inline_edit");
else 
	$templatefile = GetTemplateName("team_leader", "edit");


$keys = array();
$savedKeys = array();
$keysChanged = false;
$keys["POS_Number"] = postvalue("editid1");
$savedKeys["POS_Number"] = postvalue("editid1");
...
//	processing POS_Number - begin
	$condition = 1;

	if($condition)
	{
		$control_POS_Number = $pageObject->getControl("POS_Number", $id);
		$control_POS_Number->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		//	update key value
		if($control_POS_Number->getWebValue()!==false)
			$keys["POS_Number"] = $control_POS_Number->getWebValue();
	}
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["POS_Number"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	HeaderRedirect("team_leader", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("team_leader", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
//	show readonly fields
	...
	$xt->assign("show_key1", $pageObject->showDBValue("POS_Number", $data));


if($readevalues)
{
	$data["Team_Leader_Name"] = $evalues["Team_Leader_Name"];
	$data["POS_Number"] = $evalues["POS_Number"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("POS_Number_label","<label for=\"".GetInputElementId("POS_Number", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("team_leader", "view", $viewlink)."'\"");
