...
$page_layouts["returned_iccid_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("returned_iccid", "list", "a=return");
	exit();
}
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "returned_iccid";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("returned_iccid", "edit", $editlink)."'\"");
