...
$page_layouts["pos_out_export"] = $layout;
...
$pageObject->display(GetTemplateName("pos_out", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["ORIGIANL_CUSTOMER"] = $pageObject->getExportValue("ORIGIANL_CUSTOMER", $row);
			$values["ORG_SERVICE_NUMBER"] = $pageObject->getExportValue("ORG_SERVICE_NUMBER", $row);
			$values["CHARGE"] = $pageObject->getExportValue("CHARGE", $row);
			$values["DESTINATION_CUSTOMER"] = $pageObject->getExportValue("DESTINATION_CUSTOMER", $row);
			$values["DEST_SERVICE_NUMBER"] = $pageObject->getExportValue("DEST_SERVICE_NUMBER", $row);
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"ORIGIANL_CUSTOMER\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"ORG_SERVICE_NUMBER\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"CHARGE\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"DESTINATION_CUSTOMER\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"DEST_SERVICE_NUMBER\"";
	echo $outstr;
	echo "\r\n";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["ORIGIANL_CUSTOMER"] = $pageObject->getViewControl("ORIGIANL_CUSTOMER")->getExportValue($row, "");
			$values["ORG_SERVICE_NUMBER"] = $pageObject->getViewControl("ORG_SERVICE_NUMBER")->getExportValue($row, "");
			$values["CHARGE"] = $pageObject->getViewControl("CHARGE")->getExportValue($row, "");
			$values["DESTINATION_CUSTOMER"] = $pageObject->getViewControl("DESTINATION_CUSTOMER")->getExportValue($row, "");
			$values["DEST_SERVICE_NUMBER"] = $pageObject->getViewControl("DEST_SERVICE_NUMBER")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ORIGIANL_CUSTOMER"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ORG_SERVICE_NUMBER"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["CHARGE"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DESTINATION_CUSTOMER"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DEST_SERVICE_NUMBER"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","TRANSFER_DATE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","ORIGIANL_CUSTOMER")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","ORG_SERVICE_NUMBER")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","CHARGE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","DESTINATION_CUSTOMER")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_out","DEST_SERVICE_NUMBER")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("pos_out","TRANSFER_DATE")."</td>";
		echo "<td>".GetFieldLabel("pos_out","ORIGIANL_CUSTOMER")."</td>";
		echo "<td>".GetFieldLabel("pos_out","ORG_SERVICE_NUMBER")."</td>";
		echo "<td>".GetFieldLabel("pos_out","CHARGE")."</td>";
		echo "<td>".GetFieldLabel("pos_out","DESTINATION_CUSTOMER")."</td>";
		echo "<td>".GetFieldLabel("pos_out","DEST_SERVICE_NUMBER")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["ORIGIANL_CUSTOMER"]);
					else
						echo $values["ORIGIANL_CUSTOMER"];
			echo '</td>';
							echo '<td>';
			
									echo $values["ORG_SERVICE_NUMBER"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["CHARGE"]);
					else
						echo $values["CHARGE"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DESTINATION_CUSTOMER"]);
					else
						echo $values["DESTINATION_CUSTOMER"];
			echo '</td>';
							echo '<td>';
			
									echo $values["DEST_SERVICE_NUMBER"];
			echo '</td>';
			echo "</tr>";
		}
