...
$page_layouts["activated_sims_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("Subscriber_ICCID", $strTableName, false)."=".make_db_value("Subscriber_ICCID",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "returned_msisdn")
{
	$where = "";
		$where .= GetFullFieldName("Subscriber_Number", $strTableName, false)."=".make_db_value("Subscriber_Number",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	Subscriber_ICCID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Subscriber_ICCID", $data, $keylink);
			$row["Subscriber_ICCID_value"] = $value;
			$format = $pSet->getViewFormat("Subscriber_ICCID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Subscriber_ICCID")))
				$class = ' rnr-field-number';
			$row["Subscriber_ICCID_class"] = $class;
	//	Subscriber_Number - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Subscriber_Number", $data, $keylink);
			$row["Subscriber_Number_value"] = $value;
			$format = $pSet->getViewFormat("Subscriber_Number");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Subscriber_Number")))
				$class = ' rnr-field-number';
			$row["Subscriber_Number_class"] = $class;
	//	Event_date - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("activated_sims", "detailspreview"));
