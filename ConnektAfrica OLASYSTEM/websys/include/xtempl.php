...
//			if(method_exists($var["object"],$method))
	...
	function prepareContainers()
	{
		...
		if(!isset($page_layouts[$this->template_file]))
			return;
//	set page style
		$clayout =& $page_layouts[$this->template_file];
		$pageStyle = $clayout->style;
		if(isMobile() && $bUseMobileStyleOnly)
		{
			$pageStyle = $clayout->styleMobile;
		}
		else if(postvalue("pdf"))
		{
			$pageStyle = $clayout->pdfStyle();
		}
		
		$classPrefix = "rnr-";
		if($clayout->version == 1)
		{
			$classPrefix = "runner-";
		}
		$this->assign("stylename",$pageStyle." page-".$clayout->name);
		...
		$containersNames = array_keys($clayout->containers);
		$containersNames = array_reverse($containersNames);
		foreach($containersNames as $cname)
		{
			$container = $clayout->containers[$cname];
			...
			{
				$styleString = "";
				if(isset($clayout->skins[$cname]))
				{
					$skin = @$clayout->skins[$cname];
					
					$buttonsType = $clayout->skinsparams[$skin]["button"];
					$buttonsClass = $buttonsType == "button2" ? " aslinks" : " asbuttons";
					
					if($clayout->version == 1) {
						$styleString = " class=\"rnr-cw-".$cname." runner-s-".$skin." ".$pageStyle;
					} else {
						$styleString = " class=\"rnr-cw-".$cname." rnr-s-".$skin.$buttonsClass." ".$pageStyle;
					}
					if($hide)
					{
						$styleString .= " ".$classPrefix."hiddencontainer";
		...
		//	display blocks
		foreach($clayout->blocks as $bname=>$block)
		{
			$show = false;
			$hide = true;
	...
	{
		global $strTableName, $pageName, $page_layouts;	
		$clayout =& $page_layouts[$this->template_file];
		$menuparams = array();
		foreach($params as $p)
		{
		...
		if(($pageType == PAGE_MENU) || $countLinks>1 || $countGroups>0)
		{
			...
			else if($isMobyleLayout && $clayout->version != 1)
...
{
	echo runner_htmlspecialchars(GetFieldLabel($params["custom1"],$params["custom2"]));
}
