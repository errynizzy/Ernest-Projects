...
function checkTableName($shortTName, $type=false)
{
	...
	if ("activators" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("controller" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("form_tracking" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Custom_Report" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Simcards_in_Outlets" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pos_visits" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("simcards_batches_query" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("simcards" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Copy_of_batches" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("pos_live_balance" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("outside_stock" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pos_out" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("scratch_cards_distribution_activities" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("scratch_cards_distribution_records" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("scratch_cards" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("activators");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="activators";
	}
	$strPerm = GetUserPermissions("controller");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="controller";
	}
	$strPerm = GetUserPermissions("form_tracking");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="form_tracking";
	}
	$strPerm = GetUserPermissions("Custom Report");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Custom Report";
	}
	$strPerm = GetUserPermissions("Simcards in Outlets");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Simcards in Outlets";
	}
	$strPerm = GetUserPermissions("pos_visits");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="pos_visits";
	}
	$strPerm = GetUserPermissions("simcards_batches_query");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="simcards_batches_query";
	}
	$strPerm = GetUserPermissions("simcards");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="simcards";
	}
	$strPerm = GetUserPermissions("Copy of batches");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="Copy of batches";
	}
	$strPerm = GetUserPermissions("pos_live_balance");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="pos_live_balance";
	}
	$strPerm = GetUserPermissions("outside_stock");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="outside_stock";
	}
	$strPerm = GetUserPermissions("pos_out");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="pos_out";
	}
	$strPerm = GetUserPermissions("scratch_cards_distribution_activities");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="scratch_cards_distribution_activities";
	}
	$strPerm = GetUserPermissions("scratch_cards_distribution_records");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="scratch_cards_distribution_records";
	}
	$strPerm = GetUserPermissions("scratch_cards");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="scratch_cards";
	}
...
function GetUserPermissionsStatic($table="")
{
	...
	if($table == "activators" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "activators" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "activators" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	...
	if($table == "controller" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "controller" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "controller" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	...
	if($table == "form_tracking" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "form_tracking" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "form_tracking" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "form_tracking")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "Custom Report" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "Custom Report" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "Custom Report" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "Custom Report")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "Simcards in Outlets" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "Simcards in Outlets" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "Simcards in Outlets" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "Simcards in Outlets")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "pos_visits" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "pos_visits" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "pos_visits" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	...
	if($table == "simcards_batches_query" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "simcards_batches_query" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "simcards_batches_query" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "simcards_batches_query")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "simcards" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "simcards" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "simcards" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	...
	if($table == "Copy of batches" && $sUserGroup == "1")
	{
			return "".$extraPerm;
	}
	if($table == "Copy of batches" && $sUserGroup == "2")
	{
			return "".$extraPerm;
	}
	if($table == "Copy of batches" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "Copy of batches")
	{
		return "".$extraPerm;
	}
	if($table == "pos_live_balance" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "pos_live_balance" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "pos_live_balance" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "pos_live_balance")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "outside_stock" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "outside_stock" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "outside_stock" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "outside_stock")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "pos_out" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "pos_out" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "pos_out" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	...
	if($table == "scratch_cards_distribution_activities" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "scratch_cards_distribution_activities" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "scratch_cards_distribution_activities" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "scratch_cards_distribution_activities")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "scratch_cards_distribution_records" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "scratch_cards_distribution_records" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "scratch_cards_distribution_records" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "scratch_cards_distribution_records")
	{
		return "AEDSPI".$extraPerm;
	}
	if($table == "scratch_cards" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "scratch_cards" && $sUserGroup == "2")
	{
			return "AESP".$extraPerm;
	}
	if($table == "scratch_cards" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
	if($table == "scratch_cards")
	{
		return "AEDSPI".$extraPerm;
	}
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password)
{
	...
	$_SESSION["_activators_OwnerID"] = $data["ID"];
		$_SESSION["_controller_OwnerID"] = $data["ID"];
		$_SESSION["_Custom Report_OwnerID"] = $data["ID"];
		$_SESSION["_Simcards in Outlets_OwnerID"] = $data["ID"];
		$_SESSION["_pos_visits_OwnerID"] = $data["ID"];
		$_SESSION["_simcards_OwnerID"] = $data["ID"];
...
function detectMobileDevice()
{
	...
	{ 	
		$status = MobileDetected();
		$_SESSION['isMobile'] = ( $status >= 1 && $status <= 4 );
	}
