...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="simcards_batches_query" && $field=="New_MSISDN")
	{
		if($value == 0){
$value = '';
}
...
$value;
		...
		return $value;
				...
				if($table=="outside_stock" && $field=="New_MSISDN")
	{
		if($value == 0){
$value = '';
}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="activators" && $field=="registration_date") 
	{
		return $d = date('m/d/Y');
echo $d;;
	}
				if($table=="simcards_batches_query" && $field=="Activation_Commission") 
	{
		return $a  = 'Paid';
echo $a;;
	}
				if($table=="simcards_batches_query" && $field=="Form_Commission") 
	{
		return "To be paid";
	}
				if($table=="simcards" && $field=="Date") 
	{
		return $d = date('m/d/Y');
echo $d;
;
	}
				if($table=="outside_stock" && $field=="Activation_Commission") 
	{
		return $a  = 'Paid';
echo $a;;
	}
 ...
 * @intellisense
 */
function DoInsertRecord($table,&$avalues,&$blobfields, $pageid, &$pageObject, &$cipherer, &$keys)
{
	return DoInsertRecordSQL($table,$avalues,$blobfields, $pageid, $pageObject, $cipherer, $keys);
}
/**
 * @intellisense
 */
...
function OpenCSVFile($uploadfile)
{
