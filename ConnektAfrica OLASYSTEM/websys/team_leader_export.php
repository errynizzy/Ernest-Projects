...
$page_layouts["team_leader_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["POS_Number"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys = array();
			$keys["POS_Number"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
...
$pageObject->display(GetTemplateName("team_leader", "export"));
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("team_leader","Team_Leader_Name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("team_leader","POS_Number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("team_leader","Sales_Coordinator_Name")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("team_leader","Team_Leader_Name")."</td>";
		echo "<td>".GetFieldLabel("team_leader","POS_Number")."</td>";
		echo "<td>".GetFieldLabel("team_leader","Sales_Coordinator_Name")."</td>";
	}
	echo "</tr>";
