...
$page_layouts["batches_export"] = $layout;
...
$pageObject->display(GetTemplateName("batches", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Given_Batch_Number"] = $pageObject->getExportValue("Given_Batch_Number", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Given_Batch_Number"] = $pageObject->getViewControl("Given_Batch_Number")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Given_Batch_Number"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("batches","ID")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("batches","Batch_Number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("batches","Given_Batch_Number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("batches","Batch_size")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("batches","Status")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("batches","ID")."</td>";
		echo "<td>".GetFieldLabel("batches","Batch_Number")."</td>";
		echo "<td>".GetFieldLabel("batches","Given_Batch_Number")."</td>";
		echo "<td>".GetFieldLabel("batches","Batch_size")."</td>";
		echo "<td>".GetFieldLabel("batches","Status")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Given_Batch_Number"]);
					else
						echo $values["Given_Batch_Number"];
			echo '</td>';
							echo '<td>';
