...
$page_layouts["scratch_cards_distribution_activities_view"] = $layout;
...
$keys = array();
$keys["Serial_No"] = postvalue("editid1");
...
if (!sizeof($data)) 
{
	HeaderRedirect("scratch_cards_distribution_activities", "list", "a=return");
	exit();
}
...
$arr = array();
$arr['fName'] = "Dist_ID";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Dist_ID");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Dist_Date";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Dist_Date");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Serial_No";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Serial_No");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Full_Name";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Full_Name");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "POS_ALT_Number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("POS_ALT_Number");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Area";
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Alt_Number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Alt_Number");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Denomination";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Denomination");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Used";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Used");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Date_Used";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Date_Used");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Customer_Used";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Customer_Used");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "DSO_Name";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("DSO_Name");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "DSO_Number";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("DSO_Number");
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", $pageObject->showDBValue("Serial_No", $data));

	$keylink = "";
	$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["Serial_No"]));

	$value = $pageObject->showDBValue("Dist_ID", $data, $keylink);
	if( $mainTableOwnerID == "Dist_ID" )
		$ownerIdValue = $value;
	$xt->assign("Dist_ID_value",$value);
	if(!$pageObject->isAppearOnTabs("Dist_ID"))
		$xt->assign("Dist_ID_fieldblock",true);
	else
		$xt->assign("Dist_ID_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Dist_ID");
	$value = $pageObject->showDBValue("Dist_Date", $data, $keylink);
	if( $mainTableOwnerID == "Dist_Date" )
		$ownerIdValue = $value;
	$xt->assign("Dist_Date_value",$value);
	if(!$pageObject->isAppearOnTabs("Dist_Date"))
		$xt->assign("Dist_Date_fieldblock",true);
	else
		$xt->assign("Dist_Date_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Dist_Date");
	$value = $pageObject->showDBValue("Serial_No", $data, $keylink);
	if( $mainTableOwnerID == "Serial_No" )
		$ownerIdValue = $value;
	$xt->assign("Serial_No_value",$value);
	if(!$pageObject->isAppearOnTabs("Serial_No"))
		$xt->assign("Serial_No_fieldblock",true);
	else
		$xt->assign("Serial_No_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Serial_No");
	$value = $pageObject->showDBValue("Full_Name", $data, $keylink);
	if( $mainTableOwnerID == "Full_Name" )
		$ownerIdValue = $value;
	$xt->assign("Full_Name_value",$value);
	if(!$pageObject->isAppearOnTabs("Full_Name"))
		$xt->assign("Full_Name_fieldblock",true);
	else
		$xt->assign("Full_Name_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Full_Name");
	$value = $pageObject->showDBValue("POS_ALT_Number", $data, $keylink);
	if( $mainTableOwnerID == "POS_ALT_Number" )
		$ownerIdValue = $value;
	$xt->assign("POS_ALT_Number_value",$value);
	if(!$pageObject->isAppearOnTabs("POS_ALT_Number"))
		$xt->assign("POS_ALT_Number_fieldblock",true);
	else
		$xt->assign("POS_ALT_Number_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("POS_ALT_Number");
	...
	$value = $pageObject->showDBValue("Alt_Number", $data, $keylink);
	if( $mainTableOwnerID == "Alt_Number" )
		$ownerIdValue = $value;
	$xt->assign("Alt_Number_value",$value);
	if(!$pageObject->isAppearOnTabs("Alt_Number"))
		$xt->assign("Alt_Number_fieldblock",true);
	else
		$xt->assign("Alt_Number_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Alt_Number");
	$value = $pageObject->showDBValue("Denomination", $data, $keylink);
	if( $mainTableOwnerID == "Denomination" )
		$ownerIdValue = $value;
	$xt->assign("Denomination_value",$value);
	if(!$pageObject->isAppearOnTabs("Denomination"))
		$xt->assign("Denomination_fieldblock",true);
	else
		$xt->assign("Denomination_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Denomination");
	$value = $pageObject->showDBValue("Used", $data, $keylink);
	if( $mainTableOwnerID == "Used" )
		$ownerIdValue = $value;
	$xt->assign("Used_value",$value);
	if(!$pageObject->isAppearOnTabs("Used"))
		$xt->assign("Used_fieldblock",true);
	else
		$xt->assign("Used_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Used");
	$value = $pageObject->showDBValue("Date_Used", $data, $keylink);
	if( $mainTableOwnerID == "Date_Used" )
		$ownerIdValue = $value;
	$xt->assign("Date_Used_value",$value);
	if(!$pageObject->isAppearOnTabs("Date_Used"))
		$xt->assign("Date_Used_fieldblock",true);
	else
		$xt->assign("Date_Used_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Date_Used");
	$value = $pageObject->showDBValue("Customer_Used", $data, $keylink);
	if( $mainTableOwnerID == "Customer_Used" )
		$ownerIdValue = $value;
	$xt->assign("Customer_Used_value",$value);
	if(!$pageObject->isAppearOnTabs("Customer_Used"))
		$xt->assign("Customer_Used_fieldblock",true);
	else
		$xt->assign("Customer_Used_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Customer_Used");
	$value = $pageObject->showDBValue("DSO_Name", $data, $keylink);
	if( $mainTableOwnerID == "DSO_Name" )
		$ownerIdValue = $value;
	$xt->assign("DSO_Name_value",$value);
	if(!$pageObject->isAppearOnTabs("DSO_Name"))
		$xt->assign("DSO_Name_fieldblock",true);
	else
		$xt->assign("DSO_Name_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("DSO_Name");
	$value = $pageObject->showDBValue("DSO_Number", $data, $keylink);
	if( $mainTableOwnerID == "DSO_Number" )
		$ownerIdValue = $value;
	$xt->assign("DSO_Number_value",$value);
	if(!$pageObject->isAppearOnTabs("DSO_Number"))
		$xt->assign("DSO_Number_fieldblock",true);
	else
		$xt->assign("DSO_Number_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("DSO_Number");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "scratch_cards_distribution_activities";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("scratch_cards_distribution_activities", "edit", $editlink)."'\"");
