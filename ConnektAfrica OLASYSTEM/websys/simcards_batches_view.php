...
$page_layouts["simcards_batches_view"] = $layout;
...
if (!sizeof($data)) 
{
	HeaderRedirect("simcards_batches", "list", "a=return");
	exit();
}
...
$arr = array();
$arr['fName'] = "time";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("time");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Status";
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "batch_name";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("batch_name");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "ICCID";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("ICCID");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Active_MSISDN";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Active_MSISDN");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "New_MSISDN";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("New_MSISDN");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Swap_Date";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Swap_Date");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Airtime_transered";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Airtime_transered");
$fieldsArr[] = $arr;
$arr = array();
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Activation_Commission";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Activation_Commission");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Form_Returned";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Form_Returned");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Form_Status";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Form_Status");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Form_Commission";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Form_Commission");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Form_Paid_Date";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Form_Paid_Date");
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", $pageObject->showDBValue("ICCID", $data));
	...
	if( $mainTableOwnerID == "Event_date" )
		$ownerIdValue = $value;
	$xt->assign("Event_date_value",$value);
	...
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Event_date");
	$value = $pageObject->showDBValue("time", $data, $keylink);
	if( $mainTableOwnerID == "time" )
		$ownerIdValue = $value;
	$xt->assign("time_value",$value);
	if(!$pageObject->isAppearOnTabs("time"))
		$xt->assign("time_fieldblock",true);
	else
		$xt->assign("time_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("time");
	...
	$value = $pageObject->showDBValue("batch_name", $data, $keylink);
	if( $mainTableOwnerID == "batch_name" )
		$ownerIdValue = $value;
	$xt->assign("batch_name_value",$value);
	if(!$pageObject->isAppearOnTabs("batch_name"))
		$xt->assign("batch_name_fieldblock",true);
	else
		$xt->assign("batch_name_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("batch_name");
	$value = $pageObject->showDBValue("ICCID", $data, $keylink);
	if( $mainTableOwnerID == "ICCID" )
		$ownerIdValue = $value;
	$xt->assign("ICCID_value",$value);
	if(!$pageObject->isAppearOnTabs("ICCID"))
		$xt->assign("ICCID_fieldblock",true);
	else
		$xt->assign("ICCID_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("ICCID");
	$value = $pageObject->showDBValue("Active_MSISDN", $data, $keylink);
	if( $mainTableOwnerID == "Active_MSISDN" )
		$ownerIdValue = $value;
	$xt->assign("Active_MSISDN_value",$value);
	if(!$pageObject->isAppearOnTabs("Active_MSISDN"))
		$xt->assign("Active_MSISDN_fieldblock",true);
	else
		$xt->assign("Active_MSISDN_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Active_MSISDN");
	$value = $pageObject->showDBValue("New_MSISDN", $data, $keylink);
	if( $mainTableOwnerID == "New_MSISDN" )
		$ownerIdValue = $value;
	$xt->assign("New_MSISDN_value",$value);
	if(!$pageObject->isAppearOnTabs("New_MSISDN"))
		$xt->assign("New_MSISDN_fieldblock",true);
	else
		$xt->assign("New_MSISDN_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("New_MSISDN");
	$value = $pageObject->showDBValue("Swap_Date", $data, $keylink);
	if( $mainTableOwnerID == "Swap_Date" )
		$ownerIdValue = $value;
	$xt->assign("Swap_Date_value",$value);
	if(!$pageObject->isAppearOnTabs("Swap_Date"))
		$xt->assign("Swap_Date_fieldblock",true);
	else
		$xt->assign("Swap_Date_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Swap_Date");
	$value = $pageObject->showDBValue("Airtime_transered", $data, $keylink);
	if( $mainTableOwnerID == "Airtime_transered" )
		$ownerIdValue = $value;
	$xt->assign("Airtime_transered_value",$value);
	if(!$pageObject->isAppearOnTabs("Airtime_transered"))
		$xt->assign("Airtime_transered_fieldblock",true);
	else
		$xt->assign("Airtime_transered_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Airtime_transered");
	...
	$value = $pageObject->showDBValue("Activation_Commission", $data, $keylink);
	if( $mainTableOwnerID == "Activation_Commission" )
		$ownerIdValue = $value;
	$xt->assign("Activation_Commission_value",$value);
	if(!$pageObject->isAppearOnTabs("Activation_Commission"))
		$xt->assign("Activation_Commission_fieldblock",true);
	else
		$xt->assign("Activation_Commission_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Activation_Commission");
	$value = $pageObject->showDBValue("Form_Returned", $data, $keylink);
	if( $mainTableOwnerID == "Form_Returned" )
		$ownerIdValue = $value;
	$xt->assign("Form_Returned_value",$value);
	if(!$pageObject->isAppearOnTabs("Form_Returned"))
		$xt->assign("Form_Returned_fieldblock",true);
	else
		$xt->assign("Form_Returned_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Form_Returned");
	$value = $pageObject->showDBValue("Form_Status", $data, $keylink);
	if( $mainTableOwnerID == "Form_Status" )
		$ownerIdValue = $value;
	$xt->assign("Form_Status_value",$value);
	if(!$pageObject->isAppearOnTabs("Form_Status"))
		$xt->assign("Form_Status_fieldblock",true);
	else
		$xt->assign("Form_Status_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Form_Status");
	$value = $pageObject->showDBValue("Form_Commission", $data, $keylink);
	if( $mainTableOwnerID == "Form_Commission" )
		$ownerIdValue = $value;
	$xt->assign("Form_Commission_value",$value);
	if(!$pageObject->isAppearOnTabs("Form_Commission"))
		$xt->assign("Form_Commission_fieldblock",true);
	else
		$xt->assign("Form_Commission_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Form_Commission");
	$value = $pageObject->showDBValue("Form_Paid_Date", $data, $keylink);
	if( $mainTableOwnerID == "Form_Paid_Date" )
		$ownerIdValue = $value;
	$xt->assign("Form_Paid_Date_value",$value);
	if(!$pageObject->isAppearOnTabs("Form_Paid_Date"))
		$xt->assign("Form_Paid_Date_fieldblock",true);
	else
		$xt->assign("Form_Paid_Date_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Form_Paid_Date");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "simcards_batches";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("simcards_batches", "edit", $editlink)."'\"");
