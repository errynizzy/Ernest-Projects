...
$page_layouts["pos_live_balance_export"] = $layout;
...
$pageObject->display(GetTemplateName("pos_live_balance", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["BALANCE"] = $pageObject->getExportValue("BALANCE", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["BALANCE"] = $pageObject->getViewControl("BALANCE")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["BALANCE"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","pos_type")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","Activator_Number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","Activator_Name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","Area")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","BALANCE")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("pos_live_balance","EVENT_DATE")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("pos_live_balance","pos_type")."</td>";
		echo "<td>".GetFieldLabel("pos_live_balance","Activator_Number")."</td>";
		echo "<td>".GetFieldLabel("pos_live_balance","Activator_Name")."</td>";
		echo "<td>".GetFieldLabel("pos_live_balance","Area")."</td>";
		echo "<td>".GetFieldLabel("pos_live_balance","BALANCE")."</td>";
		echo "<td>".GetFieldLabel("pos_live_balance","EVENT_DATE")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["BALANCE"]);
					else
						echo $values["BALANCE"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
