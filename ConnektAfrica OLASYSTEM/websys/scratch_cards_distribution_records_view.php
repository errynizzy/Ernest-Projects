...
$page_layouts["scratch_cards_distribution_records_view"] = $layout;
...
$keys = array();
$keys["Dist_ID"] = postvalue("editid1");
...
if (!sizeof($data)) 
{
	HeaderRedirect("scratch_cards_distribution_records", "list", "a=return");
	exit();
}
...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Serial_No_From";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Serial_No_From");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Serial_No_To";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Serial_No_To");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Full_Name";
...
$arr = array();
$arr['fName'] = "Quantity";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Quantity");
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Lat";
...
while($data)
{
	$xt->assign("show_key1", $pageObject->showDBValue("Dist_ID", $data));

	$keylink = "";
	$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["Dist_ID"]));
	...
	$value = $pageObject->showDBValue("Serial_No_From", $data, $keylink);
	if( $mainTableOwnerID == "Serial_No_From" )
		$ownerIdValue = $value;
	$xt->assign("Serial_No_From_value",$value);
	if(!$pageObject->isAppearOnTabs("Serial_No_From"))
		$xt->assign("Serial_No_From_fieldblock",true);
	else
		$xt->assign("Serial_No_From_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Serial_No_From");
	$value = $pageObject->showDBValue("Serial_No_To", $data, $keylink);
	if( $mainTableOwnerID == "Serial_No_To" )
		$ownerIdValue = $value;
	$xt->assign("Serial_No_To_value",$value);
	if(!$pageObject->isAppearOnTabs("Serial_No_To"))
		$xt->assign("Serial_No_To_fieldblock",true);
	else
		$xt->assign("Serial_No_To_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Serial_No_To");
	...
	$value = $pageObject->showDBValue("Quantity", $data, $keylink);
	if( $mainTableOwnerID == "Quantity" )
		$ownerIdValue = $value;
	$xt->assign("Quantity_value",$value);
	if(!$pageObject->isAppearOnTabs("Quantity"))
		$xt->assign("Quantity_fieldblock",true);
	else
		$xt->assign("Quantity_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Quantity");
...
if($pageObject->isShowDetailTables && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		if ($dpParams['type'][$d] == "chart")
		{
			...
			$options['masterTable'] = "scratch_cards_distribution_records";
			$options['firstTime'] = 1;
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("scratch_cards_distribution_records", "edit", $editlink)."'\"");
