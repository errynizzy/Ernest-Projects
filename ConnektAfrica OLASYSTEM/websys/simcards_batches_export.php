...
$page_layouts["simcards_batches_export"] = $layout;
...
$pageObject->display(GetTemplateName("simcards_batches", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Total_Activation"] = $pageObject->getExportValue("Total_Activation", $row);
			$values["With_Airtime"] = $pageObject->getExportValue("With_Airtime", $row);
			$values["Without_Airtime"] = $pageObject->getExportValue("Without_Airtime", $row);
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"Total_Activation\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"With_Airtime\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"Without_Airtime\"";
	if($outstr!="")
		$outstr.=",";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Total_Activation"] = $row["Total_Activation"];
			$values["With_Airtime"] = $row["With_Airtime"];
			$values["Without_Airtime"] = $row["Without_Airtime"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Total_Activation"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["With_Airtime"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Without_Airtime"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","Manager_Name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","Event_date")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","Total_Activation")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","With_Airtime")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","Without_Airtime")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards_batches","Airtime_transered")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("simcards_batches","Manager_Name")."</td>";
		echo "<td>".GetFieldLabel("simcards_batches","Event_date")."</td>";
		echo "<td>".GetFieldLabel("simcards_batches","Total_Activation")."</td>";
		echo "<td>".GetFieldLabel("simcards_batches","With_Airtime")."</td>";
		echo "<td>".GetFieldLabel("simcards_batches","Without_Airtime")."</td>";
		echo "<td>".GetFieldLabel("simcards_batches","Airtime_transered")."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["Manager_Name"] = $pageObject->getViewControl("Manager_Name")->getExportValue($row, "");
					$values["Event_date"] = $pageObject->getViewControl("Event_date")->getExportValue($row, "");
					$values["Total_Activation"] = $pageObject->getViewControl("Total_Activation")->getExportValue($row, "");
					$values["With_Airtime"] = $pageObject->getViewControl("With_Airtime")->getExportValue($row, "");
					$values["Without_Airtime"] = $pageObject->getViewControl("Without_Airtime")->getExportValue($row, "");
					$values["Airtime_transered"] = $pageObject->getViewControl("Airtime_transered")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									echo $values["Total_Activation"];
			echo '</td>';
							echo '<td>';
			
									echo $values["With_Airtime"];
			echo '</td>';
							echo '<td>';
			
									echo $values["Without_Airtime"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
