...
$page_layouts["simcards_export"] = $layout;
...
$pageObject->display(GetTemplateName("simcards", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	{
		$values = array();
			$values["supplier_name"] = $pageObject->getExportValue("supplier_name", $row);
			$values["supplier_number"] = $pageObject->getExportValue("supplier_number", $row);
			$values["batch_number"] = $pageObject->getExportValue("batch_number", $row);
			$values["batch_size"] = $pageObject->getExportValue("batch_size", $row);
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"supplier_name\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"supplier_number\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"batch_number\"";
	if($outstr!="")
...
// write data rows
	...
	{
		$values = array();
			$values["supplier_name"] = $pageObject->getViewControl("supplier_name")->getExportValue($row, "");
			$values["supplier_number"] = $pageObject->getViewControl("supplier_number")->getExportValue($row, "");
			$values["batch_number"] = $pageObject->getViewControl("batch_number")->getExportValue($row, "");
			$values["batch_size"] = $pageObject->getViewControl("batch_size")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["supplier_name"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["supplier_number"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["batch_number"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["batch_size"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","supplier_name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","supplier_number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","batch_number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","batch_size")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","Date")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","pos_number")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("simcards","status")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("simcards","supplier_name")."</td>";
		echo "<td>".GetFieldLabel("simcards","supplier_number")."</td>";
		echo "<td>".GetFieldLabel("simcards","batch_number")."</td>";
		echo "<td>".GetFieldLabel("simcards","batch_size")."</td>";
		echo "<td>".GetFieldLabel("simcards","Date")."</td>";
		echo "<td>".GetFieldLabel("simcards","pos_number")."</td>";
		echo "<td>".GetFieldLabel("simcards","status")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["supplier_name"]);
				else
					echo $values["supplier_name"];//echo runner_htmlspecialchars($values["supplier_name"]); commented for bug #6823
					
			echo '</td>';
							echo '<td>';
			
									echo $values["supplier_number"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["batch_number"]);
				else
					echo $values["batch_number"];//echo runner_htmlspecialchars($values["batch_number"]); commented for bug #6823
					
			echo '</td>';
							echo '<td>';
			
									echo $values["batch_size"];
			echo '</td>';
							echo '<td>';
