...
function checkTableName($shortTName, $type=false)
{
	...
	if ("Reist" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("Reist");
...
function GetUserPermissionsStatic($table="")
{
	...
	if($table == "Reist" && $sUserGroup == "2")
	{
			return "AESPI".$extraPerm;
	}
	if($table == "Reist" && $sUserGroup == "1")
	{
			return "AEDSPI".$extraPerm;
	}
	if($table == "Reist" && $sUserGroup == "3")
	{
			return "SP".$extraPerm;
	}
	// default permissions	
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password)
{
	...
	$_SESSION["_Reist_OwnerID"] = $data["ID"];
