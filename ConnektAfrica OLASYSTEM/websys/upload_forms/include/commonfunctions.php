...
function checkTableName($shortTName, $type=false)
{
	...
	if ("returned_iccid" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("returned_msisdn" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("paid" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("returned_iccid");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="returned_iccid";
	}
	$strPerm = GetUserPermissions("returned_msisdn");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="returned_msisdn";
	}
	$strPerm = GetUserPermissions("paid");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="paid";
	}
...
function GetUserPermissionsStatic( $table )
{
	...
	if($table=="returned_iccid" && $sUserGroup=="1")
	{
			return "SPI".$extraPerm;
	}
	if($table=="returned_iccid" && $sUserGroup=="2")
	{
			return "SPI".$extraPerm;
	}
	if($table=="returned_iccid" && $sUserGroup=="3")
	{
			return "SPI".$extraPerm;
	}
//	default permissions	
	if($table=="returned_iccid")
	{
		return "SPI".$extraPerm;
	}
	if($table=="returned_msisdn" && $sUserGroup=="1")
	{
			return "SPI".$extraPerm;
	}
	if($table=="returned_msisdn" && $sUserGroup=="2")
	{
			return "SPI".$extraPerm;
	}
	if($table=="returned_msisdn" && $sUserGroup=="3")
	{
			return "SPI".$extraPerm;
	}
//	default permissions	
	if($table=="returned_msisdn")
	{
		return "SPI".$extraPerm;
	}
	if($table=="paid" && $sUserGroup=="1")
	{
			return "SPI".$extraPerm;
	}
	if($table=="paid" && $sUserGroup=="2")
	{
			return "SPI".$extraPerm;
	}
	if($table=="paid" && $sUserGroup=="3")
	{
			return "SPI".$extraPerm;
	}
//	default permissions	
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password, &$pageObject = null)
{
	...
	$_SESSION["_paid_OwnerID"] = $data["ID"];
