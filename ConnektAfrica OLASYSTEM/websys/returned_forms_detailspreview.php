...
$page_layouts["returned_forms_detailspreview"] = $layout;
...
if($rowcount) {
	...
	//	MSISDN - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("MSISDN", $data, $keylink);
			$row["MSISDN_value"] = $value;
			$format = $pSet->getViewFormat("MSISDN");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("MSISDN")))
				$class = ' rnr-field-number';
			$row["MSISDN_class"] = $class;
	//	Status - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("returned_forms", "detailspreview"));
