...
class RunnerPage
{
	...
	*/ 
	 ...
	 *
	 * @param string fName 						field name
	 * @param array vals 						dependant and main fields' values
	...
	/**
	 * Get for the dependant lookup field its main lookup control's field name 
	 ...
	 *
	 * @param string fName 	the field name
	 * @param string vals  	dependant and main fields' values
	 * @return Mixed
	 * @intellisense
	 */
...
//		make the resulting query
	...
	function buildDetailGridLinks(&$data)
	{
		...
		for($i = 0; $i < count($this->allDetailsTablesArr); $i ++) 
		{
			...
			$hrefs[] = array("id" => $this->pSet->getDPType($this->allDetailsTablesArr[$i]['dDataSourceTable']) == DP_INLINE 
				...
				, "href" => GetTableLink($dShortTable, "list", $masterquery));
		}
		return $hrefs;
	}
	...
	function assignStyleFiles( $isPdfPage = false )
	{
		if(isIE8() && !$isPdfPage)
		{
			...
			{
				$this->includes_css[$i] = GetTableLink("ie8css", "", $f);
			}
		}
	...
	*/
	...
	static function reloginAndLogoutProcess($permission)
	{
		...
		if( isLogged() && postvalue("a") != "logout")
			return;
	...
	public function getPageTitle($page, $table = "", $record = null, $settings = null)
	{
		...
		foreach( $matches[0] as $m )
		{
			...
			{
				$field = $pSet->getFieldByGoodFieldName( trim(substr( $m, 9, strlen($m) - 10 )) );
				if(!$masterRecord)
				{
					$masterRecord = $this->getMasterRecord();
				}
				$templ = str_replace($m, @$masterRecord[ $field ], $templ);
			}
			else
			{
				...
				$templ = str_replace($m, @$currentRecord[ $field ], $templ);
			}
		}
		return $templ;
	}
