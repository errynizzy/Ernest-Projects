...
class SQLStatement
{
	...
	function SQLStatement($sql, $order, $pagesize, $connection, &$params, $searchWhereClause, $searchHavingClause)
	{
		...
		if($recordsMode)
		{
			for($i=0; $i<count($this->fieldsArr); $i++)
			{
				...
				if($data[$this->fieldsArr[$i]['name']] !== null)
				{
					...
					if(!isset($s[$this->fieldsArr[$i]['name']]['count']))
						$s[$this->fieldsArr[$i]['name']]['count'] = 0;
...
class ReportLogic extends Summarable
{
    ...
    function ReportLogic($sql, $order, $connection, $pagesize, $printpagesize, &$params, $searchWhereClause, $searchHavingClause, $pageObject = null)
    {
    	RunnerApply($this, $params);
		Summarable::Summarable($params);
        $this->_connection = $connection;
		$this->_sql = new SQLStatement($sql, $order, $pagesize, $connection, $params, $searchWhereClause, $searchHavingClause);
...
class Report extends ReportLogic
{
    ...
    function Report($sql, $order, $connection, $pagesize, $printpagesize, &$params, $searchWhereClause, $searchHavingClause, $pageObject = null)
    {
    	// copy properties to object
		RunnerApply($this, $params);
		ReportLogic::ReportLogic($sql, $order, $connection, $pagesize, $printpagesize, $params, $searchWhereClause, $searchHavingClause, $pageObject);
    }
