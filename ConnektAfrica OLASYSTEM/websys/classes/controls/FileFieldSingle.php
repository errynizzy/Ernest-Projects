...
//	show current file
				...
				|| $this->pageObject->pSetEdit->getViewFormat($this->field) == FORMAT_FILE_IMAGE)
			{
				...
				{
					$disp = "<a target=\"_blank\" href=\"".runner_htmlspecialchars($uploadFolder.$value)."\">"
				...
				else 
				{
					...
					if($this->pageObject->pSetEdit->showThumbnail($this->field))
					{
						...
						$disp = "<a target=\"_blank\" href=\"".runner_htmlspecialchars($value)."\" class='zoombox zgallery'>";
						$disp.="<img";
						if(isEnableSection508())
							$disp.= " alt=\"".runner_htmlspecialchars($fileName)."\"";
						$disp.=" border=0";
						$disp.=" src=\"".runner_htmlspecialchars($thumbname)."\"></a>";
					}
					else
					{
						if($value != GetRootPathForResources("images/no_image.gif"))
						{
							...
							$imageValue = GetRootPathForResources("images/no_image.gif");
							
						if(isEnableSection508())
							$disp='<img alt=\"'.runner_htmlspecialchars($fileName).'\" src="'.runner_htmlspecialchars($imageValue).'" border=0>';
						else
							$disp='<img src="'.runner_htmlspecialchars($imageValue).'" border=0>';
						if($imageValue != GetRootPathForResources("images/no_image.gif"))
							$disp = "<a target=\"_blank\" href=\"".runner_htmlspecialchars($value)."\">".$disp."</a>";							
					}
				}
				$disp .= "<br />";
			}
