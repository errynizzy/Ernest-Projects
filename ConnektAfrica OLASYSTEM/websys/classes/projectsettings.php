...
class ProjectSettings
{
	...
	{
		$this->_table = $table;
		global $tables_data, $field_labels, $fieldToolTips, $page_titles, $detailsTablesData, $masterTablesData, $tableEvents, $cipherer;
	...
	function getAutoCompleteFields($field)
	{
		...
		if( $editPageType == PAGE_ADD || $editPageType == PAGE_EDIT && ( $this->isSeparate($field) || $this->isAutoCompleteFieldsOnEdit($field) ) )
		{
...
//	return Thumbnail prefix
	...
	function hasDependantFilter($field)
	{
		return $this->getDependantFilterName($field) != "";
	}
	
	function getDependantFilterName($field)
	{
		return $this->getFieldData($field, "dependantFilterName");
	}
	
	function getDependantFiltersNames($field)
	{
		return $this->getFieldData($field, "dependantFilters");
	}
...
function GetTableURL($table = "")
{
	...
	if("controller" == $table) 
		return "controller";
	if("form_tracking" == $table) 
		return "form_tracking";
	if("Custom Report" == $table) 
		return "Custom_Report";
	if("Simcards in Outlets" == $table) 
		return "Simcards_in_Outlets";
	if("pos_visits" == $table) 
		return "pos_visits";
	if("simcards_batches_query" == $table) 
		return "simcards_batches_query";
	if("simcards" == $table) 
		return "simcards";
	if("Copy of batches" == $table) 
		return "Copy_of_batches";
	if("pos_live_balance" == $table) 
		return "pos_live_balance";
	if("outside_stock" == $table) 
		return "outside_stock";
	if("pos_out" == $table) 
		return "pos_out";
	if("scratch_cards_distribution_activities" == $table) 
		return "scratch_cards_distribution_activities";
	if("scratch_cards_distribution_records" == $table) 
		return "scratch_cards_distribution_records";
	if("scratch_cards" == $table) 
...
{	
	...
	if(!$shortTName)
		return false;
	if("activators" == $shortTName) 
		return "activators";
	if("controller" == $shortTName) 
		return "controller";
	if("form_tracking" == $shortTName) 
		return "form_tracking";
	if("Custom_Report" == $shortTName) 
		return "Custom Report";
	if("Simcards_in_Outlets" == $shortTName) 
		return "Simcards in Outlets";
	if("pos_visits" == $shortTName) 
		return "pos_visits";
	if("simcards_batches_query" == $shortTName) 
		return "simcards_batches_query";
	if("simcards" == $shortTName) 
		return "simcards";
	if("Copy_of_batches" == $shortTName) 
		return "Copy of batches";
	if("pos_live_balance" == $shortTName) 
		return "pos_live_balance";
	if("outside_stock" == $shortTName) 
		return "outside_stock";
	if("pos_out" == $shortTName) 
		return "pos_out";
	if("scratch_cards_distribution_activities" == $shortTName) 
		return "scratch_cards_distribution_activities";
	if("scratch_cards_distribution_records" == $shortTName) 
		return "scratch_cards_distribution_records";
	if("scratch_cards" == $shortTName) 
		return "scratch_cards";
}
...
//	D
	...
	$g_defaultOptionValues["dependantFilterName"] = "";
	$g_defaultOptionValues["dependantFilters"] = array();
...
//	H
	$g_defaultOptionValues["hasDependantFilter"] = false;	
