	...
	*/
	function processFiltersWhere() 
	{
		...
		foreach($filters as $filter)
		{
			...
			$where = $this->getFilterWhereByType($filterType, $fName, $fValue , $sValue, $parentValues);
			if($where)
			{
	...
	*/
	...
	function parseRequest()
	{
		...
		else if( isset($_REQUEST["q"]) || isset($_REQUEST["qs"]) || @$_REQUEST["a"] == "integrated" || @$_REQUEST["f"] )
		{
			...
			$searchIsRunning = isset($_REQUEST["q"]) && $_REQUEST["q"] !== "" || isset($_REQUEST["qs"]) && $_REQUEST["qs"] !== "" ;	
			$this->bIsUsedSrch = $searchIsRunning;
		...
		if( $this->searchSavingEnabled ) 
		{
			...
			else if( $this->bIsUsedSrch && !$this->searchHasTheSameSearchParams() || $this->srchType == 'showall' )
	...
	public function storeSearchParamsForLogging()
	{
		...
		if( !isset($_REQUEST["saveSearch"]) && !isset($_REQUEST["deleteSearch"]) )
		{
			...
			else if( !@$_REQUEST["goto"] && !@$_REQUEST["orderby"] )
	...
	{
		if( @$_REQUEST["goto"] || @$_REQUEST["orderby"] )
			return true;
