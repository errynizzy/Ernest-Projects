...
if (!sizeof($data)) 
{
	HeaderRedirect("paid", "list", "a=return");
	exit();
}
...
$arr = array();
$arr['fName'] = "Date_Paid";
$arr['viewFormat'] = $pageObject->pSet->getViewFormat("Date_Paid");
$fieldsArr[] = $arr;
...
while($data)
{
	...
	$value = $pageObject->showDBValue("Date_Paid", $data, $keylink);
	if( $mainTableOwnerID == "Date_Paid" )
		$ownerIdValue = $value;
	$xt->assign("Date_Paid_value",$value);
	if(!$pageObject->isAppearOnTabs("Date_Paid"))
		$xt->assign("Date_Paid_fieldblock",true);
	else
		$xt->assign("Date_Paid_tabfieldblock",true);
		
	if( $hideEmptyFields && $value == "" ) 
		$pageObject->hideField("Date_Paid");
...
$xt->assign("editlink_attrs","id=\"editLink".$id."\" name=\"editLink".$id."\" onclick=\"window.location.href='".GetTableLink("paid", "edit", $editlink)."'\"");
